<?php include_once('../_header.php'); ?>
<div class="box">
    <h1>Polyclinic</h1>
        <h4>
            <small>Polyclinic Data</small>
            <div class="pull-right">
                <a href="" class="btn btn-default btn-xs"><i class="glyphicon glyphicon-refresh"></i></a>
                <a href="generate.php" class="btn btn-success btn-xs">Add Polyclinic <i class="glyphicon glyphicon-plus"></i></a> 
            </div>
        </h4>
        <form method="post" name="process">
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" id="polys">
                <thead>
                    <tr>
                        <th>No.</th>
                        <th>Poly Name</th>
                        <th>Building / Floor</th>
                        <th>
                            <center>
                                <input type="checkbox" name="select_all" id="select_all" value="">
                            </center>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $no = 1;
                    $sql_poly = mysqli_query($con, "SELECT * FROM tb_poly ORDER BY name_poly ASC") or die (mysqli_error($con));
                    if(mysqli_num_rows($sql_poly) > 0) { 
                        while($data = mysqli_fetch_array($sql_poly)) { ?>
                        <tr>
                            <td><?=$no++?>.</td>
                            <td><?=$data['name_poly']?></td>
                            <td><?=$data['dpfloor_poly']?></td>
                            <td align="center"><input type="checkbox" name="checked[]" id="checked" class="check" value="<?=$data['id_poly']?>"></td>
                        </tr>
                    <?php }
                    } else {
                        echo "<tr><td colspan=\"4\" align=\"center\">Data not found!</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
            </div>
    </form>
    <div class="box pull-right">
        <button class="btn btn-warning btn-sm" onclick="edit()"><i class="glyphicon glyphicon-edit"></i> Edit</button>
        <button class="btn btn-danger btn-sm" onclick="del()"><i class="glyphicon glyphicon-trash"></i> Delete</button>
    </div>
</div>
<script> 
        $(document).ready(function() {
        
        $('#select_all').on('click', function() {
            if(this.checked) {
                $('.check').each(function() {
                    this.checked = true;   
                })
            } else {
                $('.check').each(function() {
                    this.checked = false;                   
                })
            }
        })
        
        $('.check').on('click', function() {
            if($('.check:checked').length == $('.check').length){
                $('#select_all').prop('checked', true);
            }  else {
                $('#select_all').prop('checked', false);
            }
        })
    });
    
    function edit() {
        document.process.action = 'edit.php';
        document.process.submit();
    }
    function del() {
        var conf = confirm('You sure want to delete this data?');
        if(conf) {
            document.process.action = 'del.php';
            document.process.submit();
        }
    }
</script>
<?php include_once('../_footer.php'); ?>    